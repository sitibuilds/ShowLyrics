...
class LyricTime:
    ...
    def strptime(time_str, format="%M:%S.%m"):
        ...
        while time_str_idx < len(time_str) and format_idx < len(format):
            ...
            if f != "%":
                if t != f:
                    raise ValueError(f"Time string '{time_str}' does not match format '{format}'")
                else:                    
                    time_str_idx += 1
                    format_idx += 1
            else:
                ...
                if format_idx + 1 < len(format):
                    ...
                    if placeholder not in valid_placeholders:
                        raise ValueError("Invalid format. Unknown placeholder: %s" % placeholder)
                    ...
                    if placeholder == "%%":
                        ...
                        raise ValueError("Time string '{time_str}' does not match format '{format}'")
                    ...
                    try:
                        ...
                        if placeholder == "%M":
                            if mins == -1: mins = value
                            else: raise ValueError(f"Invalid format. Multiple instances of placeholder '{placeholder}' not allowed")
                    ...
                    raise ValueError("Invalid format. '%' must be followed by another character")
    ...
    def to_milliseconds(self):
        ...
        ms = (self.mins*60*1000) + (self.secs*1000) + self.ms
        return ms
    ...
    def __eq__(self, __value):
        ...
        if isinstance(__value, LyricTime):
            return __value.mins == self.mins and __value.secs == self.secs and __value.ms == self.ms
        return False
